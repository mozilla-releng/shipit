...
def build_releases_l10n(entries):
    return {
        Release(
            product="firefox",
            version=version,
            ...
            partial_updates=None,
            status=None,
        ): {locale: {"changeset": "default"} for locale in locales}
        for version, build_number, locales in entries
    }
...
def test_get_firefox_locales():
    ...
    releases_l10n = build_releases_l10n(
        [
            ("134.0b9", 2, ["af", "de", "fr"]),
            ("140.0esr", 1, ["de", "fr"]),
            ("140.2.0esr", 1, ["de", "fr"]),
    ...
    result = shipit_api.admin.product_details.get_firefox_locales(firefox_releases, releases_l10n, old_product_details, nightly_releases)
