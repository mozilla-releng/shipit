...
RELENGAPI_PROJECT_PERMISSION_MAPPING = {"tooltool/": "tooltool/api/", "base/tokens/": "tokens/api/", "mapper/": "mapper/api/"}
...
def initial_data():
    ...
    for permission, permission_doc in RELENGAPI_PERMISSIONS.items():
        ...
        if flask_login.current_user.has_permissions(new_permission):
            user["permissions"].append(dict(name=permission, doc=permission_doc))
    ...
    return dict(user=user, perms=RELENGAPI_PERMISSIONS)
...
class RelengapiToken(backend_common.db.db.Model):
    ...
    @property
    def permissions(self):
        return [from_relengapi_permission(permission) for permission in self._permissions.split(",") if permission]

    def to_dict(self):
        tok = dict(id=self.id, typ=self.typ, description=self.description, permissions=[str(a) for a in self.permissions], disabled=self.disabled)
        if self.user:
            tok["user"] = self.user
        return tok
...
def parse_header_relengapi(request):
    ...
    if claims["typ"] == "prm":
        ...
        if token_data:
            ...
            return RelengapiTokenUser(claims, permissions=token_data.permissions, token_data=token_data)
    elif claims["typ"] == "tmp":
        now = time.time()
        ...
        if token_data and not token_data.disabled:
            ...
            return RelengapiTokenUser(claims, permissions=token_data.permissions, token_data=token_data, authenticated_email=token_data.user)
