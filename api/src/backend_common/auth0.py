...
SETTINGS_REQUIRED = ("SECRET_KEY", "AUTH_CLIENT_ID", "AUTH_CLIENT_SECRET", "AUTH_REDIRECT_URI", "AUTH_DOMAIN")
...
def build_state(seed=None, size=8):
    ...
    h = hmac.new(msg=seed, key=flask.current_app.config.get("SECRET_KEY"))
...
def check_state(state, size=8):
    ...
    return hmac.compare_digest(state.encode("utf-8"), build_state(data[:size], size))
...
def auth0_login():
    ...
    return "https://{}/authorize?{}".format(flask.current_app.config.get("AUTH_DOMAIN"), urllib.parse.urlencode(params))
...
def auth0_check():
    ...
    assert state is not None, "Missing state in payload"
    assert check_state(state), "Invalid state value"

    code = flask.request.json.get("code")
    assert code is not None, "Missing code in payload"

    # Exchange code for tokens
    url = "https://{}/oauth/token".format(flask.current_app.config.get("AUTH_DOMAIN"))
    payload = {
    ...
    # Export values
    data = auth.json()
    return {"expires": int(time.time()) + data["expires_in"], "access_token": data["access_token"], "id_token": data["id_token"]}
